...
if ($portOwner) {
    ...
    # 首次冷启动需加载 123 期预测快照等盘上历史，实测可超过 1 分钟，窗口给足 150 秒
    $deadline = (Get-Date).AddSeconds(150)
    do {
        Start-Sleep -Milliseconds 500
        $health = Get-A23Health
    } until ($health.data.product_id -eq $expectedProductId -or (Get-Date) -gt $deadline)
    if ($health.data.product_id -ne $expectedProductId) { throw "A23 后端在 150 秒内未通过身份检查：$healthUri（首次启动加载历史数据较慢，请确认 backend/.venv 完整后重试）" }
