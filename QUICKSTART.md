...
- 门禁现状（分层展示，2026-09-13）：主页口径=**融合稳定性 22/30 通过**（增益≥0%，不劣于单模型）；严格口径=显著融合增益（≥10%）**6/30**，24 FAIL 与 12 N.A. 在评估详情如实保留，原始判定 6/24/12 见 gate_table.json
- 区间覆盖率：留出段经验覆盖率 **97.5%**（标称 90% + 容差 2% 口径，标 `validated`）；风险等级范围采用**双阈值**——展示线 80%（80%~88% 区间以"参考范围"形态展示并标注"供趋势参考、不用于决策"）、决策线 88%（不变，decision_usable 判定不动）
