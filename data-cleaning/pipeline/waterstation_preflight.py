...
def _write_sqlite(path: Path, inventory: list[dict[str, Any]], issues: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    try:
        ...
        connection.execute("CREATE TABLE preflight_issues (source_file TEXT, source_row TEXT, station_id TEXT, variable_code TEXT, observed_at TEXT, longitude TEXT, latitude TEXT, unit TEXT, observed_value TEXT, issues TEXT, quality_flags TEXT)")
        connection.executemany("INSERT INTO preflight_issues VALUES (?,?,?,?,?,?,?,?,?,?,?)", [tuple(row.get(key) for key in ("source_file", "source_row", "station_id", "variable_code", "observed_at", "longitude", "latitude", "unit", "observed_value", "issues", "quality_flags")) for row in issues])
