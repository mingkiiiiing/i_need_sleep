...
- 工程流程与历史实验：可运行，自动测试58/58通过；
- 真实太湖高频水质/藻情：尚未接入，当前为首要外部阻塞；
- 1—3天和7—15天标签：可用率均为0%，不可宣称短期业务预警已就绪；
- 遥感：场景检索和本地像元算法接口已实现，真实波段批量处理与地面校准未验收；
- 机理+AI提升10%：当前未达标，不对外作达标表述；
- SQLite：当前为可变最新状态，计划迁移为批次级不可变交付。
